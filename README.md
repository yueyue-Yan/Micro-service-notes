...
* 只能限制```单机```使用，如果是多台Nginx，则无法指定具体重启哪一台
* `./nginx -s reload`
...
* 启动测试
  ...
  * `http://192.168.64.128/ace-master` 带项目名的静态资源访问
    ...
    ![image-20230503095259127](README.assets/image-20230503095259127.png)

 ## 案例二、负载均衡

* 轮询
  * 将请求平均转发到所有服务器中，轮询操作
  * ![](README.assets/image-20211211155845109.png)
* 权重
...
* `\.`
  * 是转移字符，是后面这个点的转移字符
  * `.jpg`，如果第一个点不添加转义 ，会认为是任意字符，所以应该添加转义字符：``\.jpg``
* `|`
...
* 添加window的域名与ip地址的映射关系:修改host添加如下
  
  host位置：C:\Windows\System32\drivers\etc\host
